let ui = (function () {
	...
	function RevokeAccess() {
		let isConfirm = window.confirm(
			'This will log you out and you will need to grant app permission again. Continue?'
		);
		if (!isConfirm) return;
	...
	function trash() {
		if (!$('#in-trash').classList.contains('active')) $('#btn-menu-trash').click();
	}
	...
	function toggleInsertSnippet(persistent) {
		...
		let el = $('.search-box');
		if (typeof persistent == 'undefined') el.classList.toggle('w3-hide');
		else el.classList.toggle('w3-hide', !persistent);
		...
		if (!el.classList.contains('w3-hide')) {
			...
			if (typeof persistent === 'undefined') fileTab[activeTab].editor.env.editor.focus();
			$('#search-input').value = '';
			$('#search-input').blur();
		}
	}
	...
	function switchTab(direction = 1) {
		...
		else fid = activeTab + direction == -1 ? fileTab[fileTab.length - 1].fid : fileTab[0].fid;
	...
	function confirmClearData() {
		modal
			.confirm('This will delete all Codetmp saved files & folders on current browser. Continue?', false)
			.then(async () => {
	...
	function changeExplorerView(type) {
		if (!['list', 'grid'].includes(type)) return;
	...
	function waitMaterialIconsOverflow() {
		...
		new Promise((resolve) => {
			...
			document
				.querySelector('#preload-material')
				.parentNode.removeChild(document.querySelector('#preload-material'));
		});
	}
