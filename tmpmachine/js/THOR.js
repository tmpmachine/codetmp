...
function syncBlog() {
  ...
  document.body.appendChild(o.cel('script',{src:'https://blogger.com/feeds/'+id+'/posts/default?max-results='+data.maxPosts+'&alt=json-in-script&callback=ret&token='+oblog.authModule.auth.data.token}));
}
...
function init(blogData) {
  ...
  DB.get(blogData.blogId, function(result){
    ...
    data.blog.homepageUrl = 'https://attemp.web.app/';
  ...
  else
  {
    ...
    if (type == 'Blog')
    {
      ...
      else if (data.view.isLabelSearch)
      {
        ...
        {
          if (start == data.maxPosts)
            data.newerPageUrl = 'https://attemp.web.app/preview.html?q='+param('q').replace(/"/g,'&quot;');
          else
            data.newerPageUrl = 'https://attemp.web.app/preview.html?q='+param('q').replace(/"/g,'&quot;')+'&start='+(start-data.maxPosts);
        }
        if (data.posts[start+data.maxPosts] !== undefined)
          data.olderPageUrl = 'https://attemp.web.app/preview.html?q='+param('q').replace(/"/g,'&quot;')+'&start='+(start+data.maxPosts);
