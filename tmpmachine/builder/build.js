/* plate.html v1.33.x.1 : 18 sep 19 by tmpmachine - modified for TMPmachine builder */
...
function getDish(id, fullContent, original) {
...
function grill(meat, fullMeat, attributes) {
          ...
          if (waitSkip.length == pointer+1) {
            ...
            if (waitCallback) {
              ...
              if (content.startsWith('plate-')) {
                content = waitCallback(content.substring(6), meat, fullMeat);
                content = plate.cook(content, fullMeat);
              } else {
                content = '<!--'+plate.cook(content, fullMeat)+'-->';
              }
...
const plate = {
  tag: [],
  cook: function(meat, fullMeat, callback) {
    const attributes = {
      class: [],
      attribute: [],
      style: [],
