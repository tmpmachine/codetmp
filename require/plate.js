...
// v1.26 - 29 Mar 19 - center, form, table, tr, td, th htmlshorthand
// v1.25 - 27 Mar 19 - removing default w3-row class for div
// v1.24 - 6 Mar 19 - added ul and li tag
...
const plate = {
  ...
  cook: function(meat, callback) {
    ...
    meat = this.grill(meat, attributes);
  ...
  grill: function(meat, attributes) {
    ...
    const skips = [
      ...
      {open:'<!--', close:'-->', destroyTag: true, callback: plate.getDish, record: true}
    ];
    
    for (const tag of this.htmlShorthand)
    {
      settings.tag.push({
        short: tag[0],
        tag: tag[1],
    ...
    function stopRender(char) {
      ...
      if (mode == 'scanTag' || mode == 'scanAtt' || unClose > 0)
      {
        ...
        const newAtt = plate.generateAttributes(attributes);
                ...
                if (attMode == '')
                {
                  for (const attribute of settings.attributes)
                  {
                    if (attribute.open == char)
                    {
                      ...
                      for (const xs of this.cssShorthand.keys())
            ...
            else if (mode == 'scanAtt')
            {
              ...
              else
              {
                if (scanType == 'attribute')
                {
                  ...
                  {
                    if (this.cssShorthand[shortHandCheck[i]][0][shortHandPointer] == char)
                      match = true;
                    else
                    {
                  ...
                  if (match)
                  {
                    ...
                    if (char == ':')
                    {
                      ...
                      for (const xs of this.cssShorthand[shortHandCheck[0]][1].split(''))
                  ...
                  else
                  {
                    ...
                    else if (char == ';' || char == ' ')
                    {
                      ...
                      shortHandPointer = 0;
                      for (const key of this.cssShorthand.keys())
  ...
  getDish: function(id,fullContent) {
